#!/usr/bin/env bash
# Compile tread from source into a single self-contained binary. No runtime
# dependencies: the binary does not need bun, node or any vendored CLI to run.
#
# To install a released binary instead, see install.sh.
set -euo pipefail
